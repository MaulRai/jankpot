...
const PACK_TEXTURE := preload("res://assets/ui/card-back.png")
const PREMIUM_PACK_TEXTURE := preload("res://assets/ui/card-base.png")
...
func _ready() -> void:
	...
	_back_button.pressed.connect(func() -> void: get_tree().change_scene_to_file(MAIN_MENU_SCENE_PATH))
...
func _populate_shop() -> void:
	...
	_offers_grid.add_child(_create_offer({
		...
		"price": 8,
		"description": "Contains basic weapons.",
	...
	_offers_grid.add_child(_create_offer({
		"name": "Premium Pack",
		"price": 15,
		"description": "Higher chance for upgraded weapons.",
	...
	return [
		{
			"name": "Magic Ball",
			"price": 4,
			"description": "Predicts the enemy's next weapon.",
			"texture": MAGIC_BALL_TEXTURE,
			"kind": "consumable",
		},
		{
			"name": "Shield",
			"price": 2,
			"description": "Blocks 1 DMG.",
		...
		},
		{
			"name": "Remedy Kit",
			"price": 2,
			"description": "Removes Bleed.",
			"texture": REMEDY_KIT_TEXTURE,
			"kind": "consumable",
		},
		{
			"name": "Cup-a-Joe",
			"price": 2,
			"description": "Win attacks twice this turn.",
			"texture": CUP_A_JOE_TEXTURE,
			"kind": "consumable",
		},
	]
...
func _create_offer(data: Dictionary) -> Control:
	...
	buy_button.add_theme_stylebox_override("normal", StyleBoxEmpty.new())
	buy_button.add_theme_stylebox_override("hover", StyleBoxEmpty.new())
	buy_button.add_theme_stylebox_override("pressed", StyleBoxEmpty.new())
	...
	buy_button.pressed.connect(_on_buy_pressed.bind(buy_button, buy_frame, int(data["price"])))
...
func _on_button_up(button: Button, frame: PixelFramePanel) -> void:
	var target := HOVER_SCALE if button.is_hovered() else Vector2.ONE
	_tween_frame(frame, target, 0.1)


func _on_buy_pressed(button: Button, frame: PixelFramePanel, price: int) -> void:
...
func _update_reset_label() -> void:
	var seconds_left := maxi(0, (_current_reset_block() + 1) * RESET_SECONDS - Time.get_unix_time_from_system())
	var hours := int(seconds_left / 3600)
	var minutes := int((seconds_left % 3600) / 60)
	var seconds := int(seconds_left % 60)
	_reset_label.text = "CONSUMABLES RESET IN %02d:%02d:%02d" % [hours, minutes, seconds]
